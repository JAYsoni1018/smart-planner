...
if not api_key:
    ...
    raise ValueError("Weather API not found.")
elif not news_key:
    raise ValueError("News API not found.")
elif not serp_key:
    raise ValueError("Serp API not found.")
...
def get_weather_city(city:str):
    ...
    try:
        ...
        return (f"Error occure during weather api request {e}")
# a=get_weather_city("Delhi")
# print(a)
...
def get_temperature(city):
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(system_instruction=system_instructions, tools=[get_weather_city])
    )
...
def get_news(topic:str):
    #api hit
    try:
        url=f"https://newsapi.org/v2/everything?q={topic}&pageSize=5&sortBy=publishedAt&apiKey={news_key}"
        response=requests.get(url)
...
# app=get_forcasted_weather("delhi")
# print(app)
def get_local_event(city:str):
    try:
        url=f"https://serpapi.com/search.json?engine=google_events&q=Events+in+{city}&hl=en&gl=us&api_key={serp_key}"
        response=requests.get(url)
        return response.json()
    except Exception as e:
        return {"error in serp api call": str(e)}

# app=get_local_event("delhi")
# print(app)
def smart_plan(city:str):
    prompt=f"""
    ...
    response= client.models.generate_content(
        ...
        contents=prompt,
        config= types.GenerateContentConfig(
            tools=[get_local_event,get_forcasted_weather]
        )
    )
